...
export const tagRepositoryAdapter: TagRepository = {
  ...
  async findOrCreate(projectId: string, name: string): Promise<Tag> {
    const { data: existing, error: findError } = await supabase
      .from('tags').select('*').eq('project_id', projectId).ilike('name', name).maybeSingle();
    if (findError) throw findError;
    if (existing) return mapTagRow(existing);
    const { data, error } = await supabase
      .from('tags').insert({ project_id: projectId, name }).select('*').single();
    if (error) throw error;
  ...
  async findTagsForTestCase(testCaseId: string): Promise<Tag[]> {
    const { data, error } = await supabase
      .from('test_case_tags').select('tag:tags(*)').eq('test_case_id', testCaseId);
    if (error) throw error;
  ...
  async setTagsForTestCase(testCaseId: string, tagIds: string[]): Promise<void> {
    ...
    const { error: insertError } = await supabase
      .from('test_case_tags').insert(tagIds.map((tagId) => ({ test_case_id: testCaseId, tag_id: tagId })));
  ...
  async findOrCreateMany(
    ...
    if (toCreate.length > 0) {
      const { data: created, error } = await supabase
        .from('tags').insert(toCreate.map((n) => ({ project_id: projectId, name: n }))).select('*');
      if (error) throw error;
  ...
  async insertIssueTags(rows: { issueId: string; tagId: string }[]): Promise<void> {
    ...
    const { error } = await supabase
      .from('issue_tags').insert(rows.map((r) => ({ issue_id: r.issueId, tag_id: r.tagId })));
    if (error) throw error;
  },
  ...
  async insertTestCaseTags(rows: { testCaseId: string; tagId: string }[]): Promise<void> {
    if (rows.length === 0) return;
    const { error } = await supabase
      .from('test_case_tags').insert(rows.map((r) => ({ test_case_id: r.testCaseId, tag_id: r.tagId })));
    if (error) throw error;
  },
};
