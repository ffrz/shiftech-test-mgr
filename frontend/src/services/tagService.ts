...
export const tagService = {
  ...
  // Batch: resolve all tags for multiple issues at once, then batch-insert junction rows.
  async saveTagsForIssueMany(
    projectId: string,
    items: { issueId: string; tagNames: string[] }[],
  ): Promise<void> {
    const nonEmpty = items.filter((i) => i.tagNames.length > 0);
    if (nonEmpty.length === 0) return;
    ...
    const tagMap = await tagRepository.findOrCreateMany(projectId, allNames);
    const junctionRows = nonEmpty.flatMap((i) =>
      i.tagNames.map((name) => ({ issueId: i.issueId, tagId: tagMap.get(name.trim())!.id })),
    );
    await tagRepository.insertIssueTags(junctionRows);
  },
