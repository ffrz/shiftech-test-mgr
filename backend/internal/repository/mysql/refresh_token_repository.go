...
// RevokeAllForUser is what the admin "revoke access" action calls (see
// profileService.revokeAccess in the frontend) — every outstanding refresh
// token for the user stops working, forcing re-login. The already-issued
// access token still works for up to its remaining TTL (max configured
// JWT_ACCESS_TTL), which is the accepted tradeoff documented in plan §5.
