...
6. `NotFound()` - 33 edges
7. `useAuthContext()` - 33 edges
8. `Conflict()` - 32 edges
9. `main()` - 28 edges
10. `queryKeys` - 23 edges
...
Cohesion: 0.05
Nodes (49): Time, Context, DB, NewTestCaseSnapshotRepository(), NewTestPlanCaseReader(), fromDomainTestRun(), Context, DB (+41 more)
...
Cohesion: 0.11
Nodes (26): AppLayout(), AppLayoutInner(), AppSidebar(), AppSidebarMask(), AppTopbar(), getUserInitial(), BreadcrumbContext, BreadcrumbContextValue (+18 more)
...
Cohesion: 0.13
Nodes (15): FromProfile(), Profile, Context, ProfileRepository, Service, NewAuthHandler(), ProjectMemberRepository, GoogleCallbackRequest (+7 more)
...
Cohesion: 0.39
Nodes (5): Error, Kind, As(), NotFound(), Unauthorized()

### Community 127 - "scripts/generate-icons.mjs"
Cohesion: 0.29
Nodes (6): buf, __dirname, pngSrc, publicDir, root, sizes
...
- **Are the 33 inferred relationships involving `BindAndValidate()` (e.g. with `Validation()` and `.Create()`) actually correct?**
  _`BindAndValidate()` has 33 INFERRED edges - model-reasoned connections that need verification._